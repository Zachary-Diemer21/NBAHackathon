...
def value(defDist, shotDist, Final = Final)
...
for i in range(len(raw)):
    ...
    if result != False:
    str(result)
    offpl = raw[i][ (4*result[0]) + 12)]
    defpl =raw[i][ (4*result[1]) + 12)]
    offpl = value((distpoints(result[3]-result[2]) , distToBasket(ballPos))
    defpl = offpl*-1
    #player1.playerO(offpl, offpl)
#test code below
'''
time1 = time.time()
for i in range(10):
